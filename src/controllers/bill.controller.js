...
export const getBills = async (req, res) => {
    try {
      const bills = await billModel.find().populate('patientId doctorId insuranceId');
      res.status(200).json({
        success: true,
        data: bills,
      });
...
export const getBillById = async (req, res) => {
    try {
      const bill = await billModel.findById(req.params.id).populate('patientId doctorId insuranceId');
      if (!bill) {
